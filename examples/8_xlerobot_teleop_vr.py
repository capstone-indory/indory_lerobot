...
class SimpleTeleopArm:
    ...
    def handle_vr_input(self, vr_goal, gripper_state):
        ...
        """
        result = {"pose": False, "gripper": False}
        if vr_goal is None:
            return result

        metadata = getattr(vr_goal, "metadata", {}) or {}
        mode = getattr(vr_goal, "mode", None)
        mode_value = getattr(mode, "value", mode)

        if "trigger" in metadata:
            trigger_active = metadata.get("trigger", 0) > 0.5
            if self.last_trigger_active is None or trigger_active != self.last_trigger_active:
                result["gripper"] = True
                self.last_trigger_active = trigger_active
        ...
        if metadata.get("source") == "vr_grip_release" or mode_value == "idle":
            self.reset_vr_reference()
            return result
        ...
        if not hasattr(self, 'prev_vr_pos'):
            ...
            return result  # Skip first frame to establish baseline
    ...
    def gripper_action(self, robot):
        obs = robot.get_observation()
        current = obs[f"{self.prefix}_arm_gripper.pos"]
        target = self.target_positions["gripper"]
        return {f"{self.joint_map['gripper']}.pos": current + self.kp * (target - current)}
...
class SimpleHeadControl:
    ...
    def handle_headset_input(self, headset_goal):
        metadata = getattr(headset_goal, "metadata", {}) if headset_goal is not None else {}
        metadata = metadata or {}
        head_control_active = metadata.get("head_control_active", False)
        if not head_control_active:
            self.headset_origin = None
            self.target_origin = None
            return False
        ...
        if self.headset_origin is None:
            self.headset_origin = {"yaw": yaw, "pitch": pitch}
            self.target_origin = self.target_positions.copy()
            return True

        yaw_delta = _wrap_degrees(yaw - self.headset_origin["yaw"])
        pitch_delta = _wrap_degrees(pitch - self.headset_origin["pitch"])

        self.target_positions["head_motor_1"] = self.target_origin["head_motor_1"] + self.pan_gain * yaw_delta
        self.target_positions["head_motor_2"] = self.target_origin["head_motor_2"] + self.tilt_gain * pitch_delta
        return True
...
def make_head_camera_frame_provider(robot):
    def provider():
        camera = robot.cameras.get("head")
        if camera is None or not camera.is_connected:
            return None
        frame = camera.async_read(timeout_ms=50)
        if frame is None:
            return None
        camera_config = robot.config.cameras.get("head")
        color_mode = getattr(camera_config, "color_mode", "")
...
def main():
    ...
    try:
        ...
        vr_monitor.set_head_camera_frame_provider(make_head_camera_frame_provider(robot))
        ...
        try:
            while True:
                ...
                head_active = head_control.handle_headset_input(headset_goal)

                # A button is the head-camera clutch. While held, headset yaw/pitch
                # controls the head camera and arm motion is suppressed.
                if head_active:
                    left_arm.reset_vr_reference()
                    right_arm.reset_vr_reference()
                    left_state = {"pose": False, "gripper": False}
                    right_state = {"pose": False, "gripper": False}
                else:
                    # Grip/squeeze is the arm clutch: without it, only gripper and base commands are sent.
                    left_state = left_arm.handle_vr_input(left_goal, gripper_state=None)
                    right_state = right_arm.handle_vr_input(right_goal, gripper_state=None)
                
                # Get actions from both arms and head
                if left_state["pose"]:
                    left_action = left_arm.p_control_action(robot)
                elif left_state["gripper"]:
                    left_action = left_arm.gripper_action(robot)
                else:
                    left_action = {}

                if right_state["pose"]:
                    right_action = right_arm.p_control_action(robot)
                elif right_state["gripper"]:
                    right_action = right_arm.gripper_action(robot)
                else:
                    right_action = {}

                head_action = head_control.p_control_action(robot) if head_active else {}
