...
def _clamp_position_targets_to_calibration(
    bus: FeetechMotorsBus, targets: dict[str, float]
    ...
    for motor, value in targets.items():
        ...
        if clamped_value != numeric_value:
            clamped_ranges[motor] = (low, high)

    return clamped, clamped_ranges


def _nearest_periodic_raw_goal(present: int, goal: int, period: int) -> int:
    """Return the equivalent raw goal closest to the current raw position."""
    if period <= 0:
        raise ValueError("period must be positive")

    offset = round((present - goal) / period)
    candidates = (goal + (offset + delta) * period for delta in (-1, 0, 1))
    return min(candidates, key=lambda candidate: abs(candidate - present))
...
def _pop_periodic_raw_goal(
    ...
    period = bus.model_resolution_table[model]
    try:
...
class XLerobot(Robot):
    ...
    def configure(self):
        ...
        # bus 1
        self.bus1.disable_torque(num_retry=3)
        self.bus1.configure_motors(maximum_acceleration=80, acceleration=80)

        # bus 2
        self.bus2.disable_torque(num_retry=3)
        self.bus2.configure_motors(maximum_acceleration=80, acceleration=80)
        ...
        self.bus1.enable_torque(num_retry=3)
        self.bus2.enable_torque(num_retry=3)
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        left_arm_pos_raw, left_clamped = _clamp_position_targets_to_calibration(self.bus1, left_arm_pos_raw)
        right_arm_pos_raw, right_clamped = _clamp_position_targets_to_calibration(self.bus2, right_arm_pos_raw)
        head_pos_raw, head_clamped = _clamp_position_targets_to_calibration(self.bus1, head_pos_raw)
        left_wrist_roll_norm_goal, left_wrist_roll_goal_raw = _pop_periodic_raw_goal(
            self.bus1, left_arm_pos_raw, "left_arm_wrist_roll"
        )
        right_wrist_roll_norm_goal, right_wrist_roll_goal_raw = _pop_periodic_raw_goal(
            self.bus2, right_arm_pos_raw, "right_arm_wrist_roll"
        )

        if left_clamped or right_clamped or head_clamped:
            self.logs["clamped_position_targets"] = {
                **{motor: bounds for motor, bounds in left_clamped.items()},
                **{motor: bounds for motor, bounds in right_clamped.items()},
                **{motor: bounds for motor, bounds in head_clamped.items()},
            }
        else:
            self.logs.pop("clamped_position_targets", None)
        
        # Only sync_write if there are motors to write to
        if left_arm_pos_raw:
            self.bus1.sync_write("Goal_Position", left_arm_pos_raw)
        if left_wrist_roll_goal_raw is not None:
            self.bus1.sync_write(
                "Goal_Position",
                {"left_arm_wrist_roll": left_wrist_roll_goal_raw},
                normalize=False,
            )
        if right_arm_pos_raw:
            self.bus2.sync_write("Goal_Position", right_arm_pos_raw)
        if right_wrist_roll_goal_raw is not None:
            self.bus2.sync_write(
                "Goal_Position",
                {"right_arm_wrist_roll": right_wrist_roll_goal_raw},
                normalize=False,
            )
        if head_pos_raw:
        ...
        return {
            **{f"{motor}.pos": value for motor, value in left_arm_pos_raw.items()},
            **(
                {"left_arm_wrist_roll.pos": left_wrist_roll_norm_goal}
                if left_wrist_roll_goal_raw is not None
                else {}
            ),
            **{f"{motor}.pos": value for motor, value in right_arm_pos_raw.items()},
            **(
                {"right_arm_wrist_roll.pos": right_wrist_roll_norm_goal}
                if right_wrist_roll_goal_raw is not None
                else {}
            ),
            **{f"{motor}.pos": value for motor, value in head_pos_raw.items()},
            **base_goal_vel,
        }
