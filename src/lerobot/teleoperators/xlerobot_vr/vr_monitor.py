...
XLEVR_PATH = "/home/pi/XLeRobot/XLeVR"
...
class SimpleAPIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if request_path == '/api/head-camera.jpg':
            self.serve_head_camera()
    ...
    def serve_head_camera(self):
        """Serve the latest robot head camera frame as a JPEG."""
        provider = getattr(self.server, 'head_camera_frame_provider', None)
        if provider is None:
            self.send_error(404, "Head camera provider not configured")
            return

        try:
            jpeg_bytes = provider()
            if not jpeg_bytes:
                self.send_error(503, "Head camera frame unavailable")
                return
            ...
            print(f"Error serving head camera frame: {e}")
...
class SimpleHTTPSServer:
    ...
    def __init__(self, config, head_camera_frame_provider=None):
        self.config = config
        self.httpd = None
        self.server_thread = None
        self.web_root_path = XLEVR_PATH
        self.head_camera_frame_provider = head_camera_frame_provider
    ...
    async def start(self):
        ...
        try:
            ...
            self.httpd.head_camera_frame_provider = self.head_camera_frame_provider
...
class VRMonitor:
    ...
    def __init__(self):
        ...
        self.head_camera_frame_provider = None

    def set_head_camera_frame_provider(self, provider):
        self.head_camera_frame_provider = provider
    ...
    def initialize(self):
        ...
        try:
            self.https_server = SimpleHTTPSServer(
                self.config,
                head_camera_frame_provider=self.head_camera_frame_provider,
            )
        except Exception as e:
